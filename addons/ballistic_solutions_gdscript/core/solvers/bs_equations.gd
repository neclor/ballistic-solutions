...
const _SCRIPT: GDScript = BsEquations
...
## [b]Returns:[/b] The firing velocity vector, or a NaN vector if [param time] ≤ 0.
static func velocity(time: float, to_target: Vector4, target_velocity: Vector4 = Vector4.ZERO, projectile_acceleration: Vector4 = Vector4.ZERO, target_acceleration: Vector4 = Vector4.ZERO) -> Vector4:
	if time < 0 or is_zero_approx(time):
		_BsLogger.format_error(_SCRIPT, BsEquations.velocity, "Zero or negative `time`", "nan vector")
