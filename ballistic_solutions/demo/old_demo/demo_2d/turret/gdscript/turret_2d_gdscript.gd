...
func _physics_process(_delta: float) -> void:
	...
	impact_times = BsTime2D.all_by_speed(projectile_speed, to_target, target_velocity, projectile_acceleration, target_acceleration)
	...
	match impact_times.size():
		...
		1:
			crosshair_1.position = BsPosition2D.position(to_target, impact_times[0], target_velocity, target_acceleration)
			crosshair_2.position = Vector2.ZERO
		2:
			crosshair_1.position = BsPosition2D.position(to_target, impact_times[0], target_velocity, target_acceleration)
			crosshair_2.position = BsPosition2D.position(to_target, impact_times[1], target_velocity, target_acceleration)
...
func create_projectiles() -> void:
	...
	for time in impact_times:
		create_projectile(BsVelocity2D.velocity(time, to_target, target_velocity, projectile_acceleration, target_acceleration))
